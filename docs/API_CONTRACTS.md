...
Every error response also carries the request's correlation ID as a `correlationId` extension
member, sourced from the `X-Correlation-Id` header (generated by API Gateway if the client didn't
send one — see JD-37 and HLD 6.1):
```json
{
  "type": "about:blank",
  "title": "Not Found",
  "status": 404,
  "detail": "Application not found with id: 3fa85f64-5717-4562-b3fc-2c963f66afa6",
  "correlationId": "b3a1e6d2-9c4f-4a7e-8f0a-1d2c3b4a5e6f"
}
```
...
- `correlationId` is useful for support/debugging — surface it in error UI or logs if helpful, but it's not required for handling the error itself.
